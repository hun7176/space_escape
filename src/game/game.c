...
void draw_game_area() {
    ...
    //             mvwprintw(game_win, items[i].y, items[i].x, "✦");  // 맵 클리어 아이템
    ...
    //             mvwprintw(game_win, items[i].y, items[i].x, "B");  // 폭탄 아이템
...
void draw_ui_area() {
    ...
    if (current_score != last_score || current_opponent_score != last_opponent_score) {
        ...
        mvwprintw(ui_win, 18, 2, "Pause: [p] or Ctrl+Z");
        mvwprintw(ui_win, 19, 2, "Exit:  Ctrl+C");
